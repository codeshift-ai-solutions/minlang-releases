---
title: "Sample: Sales CRM"
parent: Cookbook
nav_order: 3
---

# Sales CRM — a pipeline with lifecycle rules

A complete two-screen CRM: companies (create-only, unique names) and deals that move through a pipeline (`lead → qualified → won/lost`) with ownership rules enforced on **every** path — create and move alike. This is the pattern to copy for any app that mixes immutable reference data with mutable workflow records.

The listing is **validator-clean as of bundle v4** (`ml1 validate` → `validation ok`). Raw file: [samples/crm-pipeline.ml](samples/crm-pipeline.ml).
...
**The create-only/mutable split is the heart of the program.** `Company` has no `set`-actions, so it may carry the input-bound `UniqueCompanyName` constraint. `Deal` is mutable (`MoveDeal`, `ReassignDeal`), so all of its constraints are `self.`-bound predicates or whole-reference joins — the forms that hold on every write path. This is the D3/D4/D7 interplay from the [style guide](../style-guide.md) applied deliberately, not worked around.

**Ownership rules guard the stages that matter, per stage.** `DealOwnerRequiredWhenQualified` and `DealOwnerRequiredWhenWon` are two constraints with two distinct messages rather than one clever predicate — each gets its own byte-exact failure test, and a reviewer can see exactly which business rule fired. A deal can be a `lead` with no owner (it's a top-of-funnel record), but it cannot be *qualified* or *won* unowned, whether that happens at create or via `MoveDeal`.

**The triads cover the lifecycle.** For the qualified-owner rule: `CreateDealSucceeds` + `DealOwnerRequiredWhenQualifiedRejectsCreate` (create path) and `MoveDealSucceeds` + `MoveDealRejectsQualifiedWithoutOwner` (set path). The rejection tests assert the state didn't move: `entity_count(Deal) == 0` after a rejected create, `entity_has(Deal.1, stage == "lead")` after a rejected move. `DealOwnerRequiredWhenWonSucceeds` proves an owned deal *can* be won — success evidence for the won-rule, not just failure evidence.
...
```bash
npm create minlang-app crm
cd crm
# replace crm.ml with the listing above
make compile && pnpm --dir app install && make test && make dev
```

Extending it is the usual loop: add `Contact` as another create-only entity under `Company`, or an `Activity` log per deal — entity, constraints, triads, recompile.
